...
class VoterModel {
    ...
    async getAllVotersAdmin(id) {
        try {
            const data = await pool.query('SELECT voter.id, email, first_name, last_name, password, has_voted, election_id, role, name FROM voter JOIN election ON voter.election_id = election.id WHERE election_id IN (SELECT election_id FROM admin_elections WHERE admin_id = $1);',[id]);
    ...
    async getAllVotersByElectionId(id) {
        try {
            const data = await pool.query('SELECT voter.id, email, first_name, last_name, password, has_voted, election_id, role, name FROM voter JOIN election ON voter.election_id = election.id WHERE election_id = $1', [id]);
            return data.rows?.length ? data.rows : null;
        } catch (error) {
            throw new Error(error);
        }
